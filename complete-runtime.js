...
(function(){
  ...
  const ADMIN_USER=(typeof TRIP_CONFIG!=='undefined'&&TRIP_CONFIG.admin&&TRIP_CONFIG.admin.user)||'';
  const ADMIN_PIN=(typeof TRIP_CONFIG!=='undefined'&&TRIP_CONFIG.admin&&TRIP_CONFIG.admin.pin)||'';
  ...
  function openTripStudioPanel(){
    if(!isAdminUser()){ alert((typeof TRIP_CONFIG!=='undefined'&&TRIP_CONFIG.admin&&TRIP_CONFIG.admin.studioMessage)||'Trip Studio access is not configured.'); return false; }
  ...
  function updateUI(){
    ...
    if(selectorCard){
      ...
      if(status) status.textContent=active?'Studio active · Press × to exit':'PIN protected · '+((TRIP_CONFIG.admin&&TRIP_CONFIG.admin.displayName)||'Admin')+' only';
  ...
  function buildShell(){
    ...
    if(familySheet && familyList && !document.getElementById('tripStudioSelectorToggle')){
      ...
      selectorToggle.innerHTML=`<span class="trip-studio-selector-copy"><strong>⚙ Studio Mode</strong><small>Editing, Complete Trip, Export Centre and trip controls</small><em class="trip-studio-selector-status">PIN protected · ${((TRIP_CONFIG.admin&&TRIP_CONFIG.admin.displayName)||'Admin')} only</em></span><span class="trip-studio-selector-arrow" aria-hidden="true">›</span>`;
  ...
  window.setAdminMode=function(enabled){
    ...
    if(enabled && !isAdminUser()){
      alert((typeof TRIP_CONFIG!=='undefined'&&TRIP_CONFIG.admin&&TRIP_CONFIG.admin.studioMessage)||'Trip Studio access is not configured.');
      updateUI();
      return false;
    }
