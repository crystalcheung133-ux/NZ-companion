...
(function(root){
  ...
  async function authRequest(path,body){const res=await fetch(config.url.replace(/\/$/,'')+path,{method:'POST',headers:{apikey:config.anonKey,'Content-Type':'application/json'},body:JSON.stringify(body||{})});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data?.msg||data?.message||`Auth ${res.status}`);return data;}
  async function getSession(){let session=readJSON(SESSION_KEY,null);if(sessionValid(session))return session;if(session?.refresh_token){try{session=await authRequest('/auth/v1/token?grant_type=refresh_token',{refresh_token:session.refresh_token});writeJSON(SESSION_KEY,session);return session;}catch(e){}}session=await authRequest('/auth/v1/signup',{});writeJSON(SESSION_KEY,session);return session;}
