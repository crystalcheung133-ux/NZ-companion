...
const suiteDir=path.join(root,'suites');
const suiteFiles=fs.existsSync(suiteDir)?walk(suiteDir).filter(p=>p.endsWith('.sh')):[];
const staticShell=[path.join(root,'run-all.sh'),...suiteFiles].map(p=>fs.readFileSync(p,'utf8')).join('\n');
