...
def run_viewport(browser,base,viewport,label):
      ...
      try:
        ...
        page.evaluate("openAccommodationDetail('fixture-hotel-booking')")
        page.wait_for_selector('#tripModal.show')
        ...
        check('Sample Hotel' in detail,label+': fixture Booking detail missing')
        check(page.locator('#tripModalContent a.trip-action-btn--book',has_text='Book Online').count()>0,
              label+': actionable Book Online channel missing')
        ...
        check('openList' in types and 'rest' in types,label+': fixture activity semantics missing')
