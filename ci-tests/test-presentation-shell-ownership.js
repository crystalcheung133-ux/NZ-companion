...
for(const x of ['--engine-fixed-chrome-height','top:var(--engine-fixed-chrome-height)!important','overflow-y:auto!important','overflow:visible!important','pointer-events:none!important','#tripStudioModal.show']) if(!css.includes(x)) fail.push('missing '+x);

if(!/#mamaModal\s*\{[^}]*position:fixed!important;[^}]*z-index:var\(--engine-traveller-layer\)!important;[^}]*isolation:isolate!important;/s.test(css) || !/#mamaModal\.show\s*\{[^}]*display:grid!important;[^}]*place-items:center!important;[^}]*pointer-events:auto!important;/s.test(css)) fail.push('canonical Traveller Selector overlay owner missing');
if(!/#mamaModal\.show > \.guide-sheet\s*\{[^}]*max-height:calc\(100% - 4px\)!important;[^}]*overflow-y:auto!important;/s.test(css)) fail.push('canonical Traveller Selector sheet owner missing');
...
if(!/#tripStudioModal\s*\{[^}]*z-index:7800!important;[^}]*isolation:isolate!important;/s.test(css)) fail.push('Studio workspace isolated overlay owner missing');
if(!/#tripStudioModal \.trip-studio\s*\{[^}]*z-index:2!important;[^}]*background:/s.test(css)) fail.push('Studio card foreground owner missing');
if(css.includes('#mamaModal.studio-view')) fail.push('historical Studio ownership must not return to Traveller Selector');
