/* RC18.1 release gate. Run with: node freeze-validation.js */
...
assert(read('VERSION.txt').trim()==='NZ Companion RC18.1 \u00b7 Freeze Blocker Remediation','VERSION.txt is not RC18.1');
const tripConfig=read('trip-config.js');
assert(/version:\s*'RC18\.1'/.test(tripConfig),'TRIP_CONFIG.version is not RC18.1');
assert(/buildLabel:\s*'Freeze Blocker Remediation'/.test(tripConfig),'TRIP_CONFIG.buildLabel is inconsistent');
assert(read('sw.js').includes('${TRIP_CONFIG.version}-freeze-blocker-remediation'),'Service Worker cache does not use canonical release identity');
assert(manifestLinks[0]&&/manifest\.webmanifest\?v=rc18-1/.test(manifestLinks[0][0]),'Active manifest reference is not RC18.1');
for(const name of htmlFiles){
  for(const match of read(name).matchAll(/(?:src|href)=["']([^"']+\.(?:js|css))\?v=([^"']+)["']/g)){
    if(/^https?:/.test(match[1]))continue;
    assert(match[2]==='nz1.0-rc18-1',`${name} has non-RC18.1 active asset generation: ${match[0]}`);
  }
}
...
console.log(`PASS RC18.1 freeze validation: ${Object.keys(places).length} places, ${guideOrder.length} Guide entries, ${items.length} itinerary items, ${items.filter(item=>item.nonPlace===true).length} non-place events.`);
