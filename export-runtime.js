/* Travel Engine v1.0 â€” RC5.0 Native Share & Preparation Checklist. */
(function(){
  'use strict';
  ...
  function buildControl(){
    ...
    section.innerHTML='<button class="trip-export-launch" type="button" onclick="openTripExportCenter()"><span><strong>Open Export Centre</strong><small>Itinerary and expenses are available anytime.</small></span><span aria-hidden="true">â€º</span></button>';
  ...
  function buildModal(){
    ...
    modal.innerHTML=`<div class="trip-export-sheet" role="dialog" aria-modal="true" aria-labelledby="tripExportTitle"><button class="trip-export-close" type="button" onclick="closeTripExportCenter()" aria-label="Close">Ã—</button><p class="kicker">TRIP OUTPUTS</p><h2 id="tripExportTitle">Export Trip</h2><p class="lead">Share through the iPhone or Android share sheet, or create a printable copy.</p><div class="trip-export-list"><button type="button" onclick="shareItineraryNative()"><span class="trip-export-icon">ðŸ“¤</span><span><strong>Share Itinerary</strong><small>Open WhatsApp, Mail, Messages, AirDrop and other apps.</small></span><span>â€º</span></button><button type="button" onclick="exportFinalItinerary()"><span class="trip-export-icon">ðŸ“„</span><span><strong>Printable Itinerary</strong><small>Open a clean copy and save it as PDF.</small></span><span>â€º</span></button><button type="button" onclick="exportExpenseSummary()"><span class="trip-export-icon">ðŸ’°</span><span><strong>Expense Summary</strong><small>Download the complete expense and settlement CSV.</small></span><span>â€º</span></button><button type="button" class="coming-soon" disabled><span class="trip-export-icon">ðŸ“–</span><span><strong>Memory Book</strong><small>Coming Soon</small></span></button></div></div>`;
  ...
  function itineraryShareText(){
    ...
    days.forEach(dayNo=>{
      ...
      lines.push(`${day.kicker||`Day ${dayNo}`} â€” ${day.heading||day.title||''}`);
      const drive=day.drive||{};
      if(drive.route) lines.push(`Drive: ${drive.route}${drive.distance?` Â· ${drive.distance}`:''}${drive.drivingTime?` Â· ${drive.drivingTime}`:''}`);
